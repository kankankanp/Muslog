...
func (u *postUsecaseImpl) UpdatePost(ctx context.Context, input UpdatePostInput) (*entity.Post, error) {
	var updatedPost *entity.Post

	err := u.txManager.Do(ctx, func(txRepo domainRepo.RepositoryProvider) error {
		// 既存Post取得（ユーザー確認付き）
		post, err := txRepo.PostRepository().FindByIDWithUserID(ctx, input.ID, input.UserID)
		if err != nil {
			return err
		}
		...
		post.HeaderImageUrl = input.HeaderImageUrl
		...
		if err := txRepo.PostRepository().Update(ctx, post); err != nil {
			return err
		}

		// Tags を再設定
		if err := txRepo.TagRepository().RemoveTagsFromPost(post.ID, nil); err != nil { // nilなら全削除扱い
			return err
		}
		if len(input.Tags) > 0 {
			if err := txRepo.TagRepository().AddTagsToPost(post.ID, input.Tags); err != nil {
	...
	// フルロードして返す
	loaded, err := u.repo.FindByID(ctx, updatedPost.ID)
	if err != nil {
